...
def get_expected_value(Q, state_index, epsilon):
    
    n_actions = Q.shape[1]
    
    greedyaction = np.argmax(Q[state_index])
    
    expected = 0
    
    for action in range(n_actions):
        if action == greedyaction:
            probaction = 1 - epsilon + (epsilon / n_actions)
        else:
            probaction = epsilon / n_actions
            
        expected += probaction * Q[state_index, action]
...
def sarsa_update(Q, s, a, r, s_next, a_next, alpha, gamma, epsilon):
    
    expected = get_expected_value(Q, s_next, epsilon)
    predict = Q[s, a]
...
def run_sarsa():
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            # update rule
            sarsa_update(Q, s_idx, a, reward, s_next_idx, a_next, alpha, gamma, epsilon)
